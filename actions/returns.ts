        ...
        const result = await db.$transaction(async (tx) => {
            ...
            const newReturn = await tx.return.create({
                data: {
                    ...
                    refundAmount,
                    status: "COMPLETED",
                    paymentType: refundPaymentType ?? (invoice as any)?.paymentType ?? "CASH",
            ...
            let creditAccount: string = "1101"; // Default Treasury
            const effectivePaymentType: PaymentType =
                refundPaymentType ?? ((invoice as any)?.paymentType as PaymentType) ?? "CASH";
            ...
            await createJournalEntry({
                tenantId: context.tenantId,
                description: `Return ${returnToken} for Invoice ${(invoice as any).token ?? invoiceId} (${effectivePaymentType})`,
...
export async function getReturnableItemsAction(invoiceId: string) {
    ...
    return {
        invoiceToken: (invoice as any)?.token ?? invoiceId,
