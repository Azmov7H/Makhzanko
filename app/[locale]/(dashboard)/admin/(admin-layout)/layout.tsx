...
export default async function OwnerLayout({
  ...
  const navItems = [
    ...
    { href: "/admin/promo-codes", icon: Ticket, label: "أكواد الترويج" },
    { href: "/admin/activity", icon: Activity, label: "سجل النشاطات" },
    { href: "/admin/support", icon: MessageCircle, label: "الدعم الفني" },
    { href: "/admin/announcements", icon: Megaphone, label: "نظام الإعلانات" },
    { href: "/admin/settings", icon: Settings, label: "الإعدادات العامة" },
    { href: "/admin/usage", icon: Zap, label: "تتبع الموارد" },
    { href: "/admin/backup", icon: Database, label: "النسخ الاحتياطي" },
    { href: "/admin/admins", icon: ShieldCheck, label: "إدارة المدراء" },
  ];
      ...
      <aside className="fixed inset-y-0 right-0 z-10 hidden w-64 flex-col border-l bg-card shadow-lg lg:flex">
        ...
        <div className="border-t p-4">
          <form
            action={async () => {
              "use server";
              const { ownerLogoutAction } = await import("@/actions/admin/auth");
